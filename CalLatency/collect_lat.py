...
# for cli_id in range(1,cli_num+1):
sequence = [2 ** i for i in range(int(math.log2(cli_num)) + 1)]
for cli_id in sequence:
    for coro_id in range(1,coro_num+1):
        # filenames = [f"{filename_prefix}_1_{cli_id}_{coro_id}{server_ip[i]}.txt" for i in range(server_count)]
        filenames = [f"{filename_prefix}_{server_count}_{cli_id}_{coro_id}.txt" for i in range(server_count)]
