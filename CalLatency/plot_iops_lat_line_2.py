...
correspond_delay = data.loc[delay_name].astype(np.float64)
if delay_name == "P999 latency" or delay_name == "P99 latency" or delay_name == "P9999 latency" :
    correspond_delay = np.log(correspond_delay)
...
for index_label, properties in colors_markers.items():
    ...
    latency_values = [correspond_delay.iloc[i][j] for j in range(len(x_labels)) if j % 4 == 0]
    selected_latency.extend(latency_values)

    # Convert the selected lists to NumPy arrays
    selected_throughputs = np.array(selected_throughputs)
    selected_latency = np.array(selected_latency)
    ...
    plt.plot(selected_throughputs, selected_latency, label=index_label, color=properties['color'],
