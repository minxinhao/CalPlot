...
fig, axes = plt.subplots(nrows=1, ncols=len(selected_delays), figsize=(12, 5.5))
...
for delay_index, delay_name in enumerate(selected_delays):
    ...
    if delay_name == "P10 latency":
        ...
        ax.text(0.5, -0.1, '(b) P999 Latency', transform=ax.transAxes, fontsize=20, va='top', ha='center')
        # ax.set_yticklabels([10, 100, 1000])
        # ax.set_yticks(np.linspace(0, float(max_value), num=6, dtype=int))  # Ensure the tick values are integers
        ax.set_yticks(np.power(10, np.arange(0, np.ceil(np.log10(max_value)), 1)))  # Set the y-tick values as powers of 10
...
plt.subplots_adjust(wspace=0.15, top=.93,bottom=.2,left=.08,right=.99)
